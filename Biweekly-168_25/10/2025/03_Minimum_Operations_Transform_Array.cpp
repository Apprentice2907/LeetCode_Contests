...
Thus, after 3 operations nums1 is transformed into nums2.©leetcode
...
// My logic and also blackbox help andd chatGPT coded and 151ms runtime
...
public:
    long long minOperations(vector<int>& nums1, vector<int>& nums2) {
        ...
        for (int j = 0; j < n; ++j) {
            total_sum += abs(nums1[j] - nums2[j]);
        }
        
        long long min_ops = LLONG_MAX;
        for (int k = 0; k < n; ++k) {
            long long sum_other = total_sum - abs(nums1[k] - nums2[k]);
            int a = nums1[k];
            int b = nums2[k];
            int c = nums2[n];
            ...
            int med = vals[1];
            long long cost = abs(a - med) + abs(b - med) + abs(c - med);
            long long total_cost = 1 + sum_other + cost;
